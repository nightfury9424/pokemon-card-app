...
class _CardDetailScreenState extends State<CardDetailScreen>
  ...
  Future<void> _onSellTapImpl(
    ...
    // (RAW 는 위 실카드 검증 게이트, GRADED 는 슬랩 게이트로 대칭. 1.0.5 드리프트 복구 #5.)
    if (cardStatus == 'GRADED' &&
        assetIdForPhotos != null &&
        assetIdForPhotos.isNotEmpty) {
  ...
  /// 등급 자산에 슬랩(SLAB) 실사진이 등록돼 있나. (판매 전 게이트용)
      ...
      builder: (sheetCtx) => StatefulBuilder(
        builder: (ctx, setSheet) {
          Future<void> pick() async {
            ...
            // ctx.mounted 가드 — 픽커 열린 사이 시트 닫히면 setSheet-after-unmount 방지.
            if (picked != null && ctx.mounted) {
              setSheet(() => slab = File(picked.path));
            }
          }
