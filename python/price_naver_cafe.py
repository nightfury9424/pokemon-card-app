...
def already_saved(conn, card_id: str, traded_at: datetime) -> bool:
    """같은 날짜 같은 카드의 NAVER_CAFE 스냅샷 중복 방지"""
    with conn.cursor() as cur:
        cur.execute("""
...
def save_price(conn, card_id: str, price: int, card_status: str,
    ...
    with conn.cursor() as cur:
        cur.execute("""
            ...
            VALUES (%s, %s, 'NAVER_CAFE', %s, %s, %s, %s, %s, NOW(), %s, %s)
        """, (snap_id, card_id, price, card_status,
...
def main():
    ...
    for page in range(1, args.max_pages + 1):
        ...
        for art in articles:
            ...
            winning_price = extract_winning_price(comments)
            ...
            if conn:
                ...
                if card_id:
                    ...
                    if not already_saved(conn, card_id, write_dt):
                        save_price(conn, card_id, winning_price, card_status,
                                   grade, grade_val, write_dt,
                                   validation_status, invalid_reason)
                        log.info(f'  → 저장 [{validation_status}'
                                 f'{" reason=" + invalid_reason if invalid_reason else ""}]: {card_id}')
                    else:
                        log.debug(f'  → 중복 스킵: {card_id}')
                    matched += 1
                else:
