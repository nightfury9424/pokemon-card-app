...
HTML = r'''<!doctype html><html><head><meta charset=utf-8><title>시리즈 갭 검수 v7</title>
<style>
...
function render(){
  ...
  document.getElementById('list').innerHTML = cards.map((d,i)=>{
    ...
    const prodCell=(d.prod_img||d.match_cid)?imgCell(d.prod_img,'기존 prod 최근접<br>'+pverdict):'<div class=imgwrap><div class=noimg style="color:#bbb">기존 prod<br>매칭 없음<br>(신규 추정)</div></div>';
...
open('series_gap_final_review_v7.html', 'w').write(HTML)
print(f"series_gap_final_review_v7.html 생성 ({len(HTML)//1024}KB)")
