...
HTML = r'''<!doctype html><html><head><meta charset=utf-8><title>시리즈 갭 검수 v5</title>
<style>
...
.imgwrap img{width:210px;height:290px;object-fit:contain;background:#f5f5f5;border:1px solid #ddd;cursor:zoom-in;display:block}
.noimg{width:210px;height:290px;display:flex;align-items:center;justify-content:center;background:#f0f0f0;border:1px dashed #aaa;color:#999}
...
let state=JSON.parse(localStorage.getItem('gapReviewV5')||'{}');
...
function save(){localStorage.setItem('gapReviewV5',JSON.stringify(state));}
...
function evidence(d){
  if(['MANUAL_REVIEW','EXCLUDE_SAME_ART_AUTO_CANDIDATE','EXCLUDE_EXACT_REF'].includes(d.cls)){
    ...
    return `<tr><td>prod 매칭</td><td>${d.match_name||'-'} (${d.match_rarity||'?'}) <code>${(d.match_cid||'').slice(0,16)}</code></td></tr>
    <tr><td>score1 / score2</td><td><b>${d.s1}</b> / ${d.s2} · margin <b>${d.margin}</b></td></tr>
    <tr><td>이름 / 레어도 일치</td><td>${nm} / ${rr}</td></tr>`;
  }
  if(d.cls==='UNRESOLVED') return `<tr><td>미해결 사유</td><td>${d.unres_reason}</td></tr>`;
  return '';
}
function render(){
  const cards=visible();
...
open('series_gap_final_review_v5.html', 'w').write(HTML)
print(f"series_gap_final_review_v5.html 생성 ({len(HTML)//1024}KB)")
