...
HTML = r'''<!doctype html><html><head><meta charset=utf-8><title>시리즈 갭 검수 v9</title>
<style>
...
function render(){
  ...
  document.getElementById('list').innerHTML = cards.map((d,i)=>{
    ...
    let pverdict='';
    if(d.cls==='EXCLUDE_EXACT_REF') pverdict='⛔ 동일 ref=중복확정';
    else if(d.s1!=null&&d.s1!=='') pverdict=(d.s1>=0.92?'🔴 동일일러스트 추정→제외후보':'🟢 다른 카드(0.92미만)→신규가능');
    const prodCell=(d.prod_img||d.match_cid)?imgCell(d.prod_img,'기존 prod 유사후보(확정 매핑 아님)<br>'+pverdict):'';
    return `<div class="card ${dec} ${i===focusIdx?'focus':''}" id=card${i}>
      ...
      ${prodCell}
      <div class=info>
        <b>${d.ko_name}</b> <span class=tag style="background:${CLSCOLOR[d.cls]}">${d.cls}</span>${(d.ko_jp!=null&&d.ko_jp<0.80)?' <span class=tag style="background:#c0392b">⚠️ 매핑오류 의심 KO↔JP '+d.ko_jp+'</span>':''}${d.cls==='UNRESOLVED'?' <span class=tag style="background:#636e72">⛔ JP ref 미해결=추가 불가</span>':((!d.prod_img&&!d.match_cid&&d.cls==='NEW_CANDIDATE')?' <span class=tag style="background:#00b894">✓ prod에 유사 카드 없음=신규</span>':'')}
        <table>
...
open('series_gap_final_review_v9.html', 'w').write(HTML)
print(f"series_gap_final_review_v9.html 생성 ({len(HTML)//1024}KB)")
