...
HTML = r'''<!doctype html><html><head><meta charset=utf-8><title>시리즈 갭 검수 v8</title>
<style>
...
function render(){
  ...
  document.getElementById('list').innerHTML = cards.map((d,i)=>{
    ...
    const prodCell=(d.prod_img||d.match_cid)?imgCell(d.prod_img,'기존 prod 유사후보(확정 매핑 아님)<br>'+pverdict):'<div class=imgwrap><div class=noimg style="color:#bbb">기존 prod<br>매칭 없음<br>(신규 추정)</div></div>';
    return `<div class="card ${dec} ${i===focusIdx?'focus':''}" id=card${i}>
      ...
      ${prodCell}
      <div class=info>
        <b>${d.ko_name}</b> <span class=tag style="background:${CLSCOLOR[d.cls]}">${d.cls}</span>${(d.ko_jp!=null&&d.ko_jp<0.80)?' <span class=tag style="background:#c0392b">⚠️ 매핑오류 의심 KO↔JP '+d.ko_jp+'</span>':''}
        <table>
...
open('series_gap_final_review_v8.html', 'w').write(HTML)
print(f"series_gap_final_review_v8.html 생성 ({len(HTML)//1024}KB)")
