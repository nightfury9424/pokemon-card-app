...
## 4. Flutter 통합 스펙
### 4.0 자산 마감 (누끼 · 실루엣컷 · 3D 입체) — 필수
- **배경 투명 누끼**: 생성물의 중립 회색 배경 **완전 제거**(검은 무대에 회색 박스/헤일로 금지).
- **실루엣 컷**: 사각 크롭 금지 — 팩 **실루엣(상하 크림프 톱니·좌우 실링 포함) 라인 따라** 컷. 톱니 사이 배경도 제거.
- **절취선 정합**: `pack_top_strip`은 **크림프/절취 라인 정확히 따라** 분리 — 그 라인이 tear 애니 경계.
- **3D 입체**: ①렌더 자체에 두께/측면광(약한 볼륨) ②Flutter에서 **원근 기울기**(`Matrix4` perspective, 드래그/자이로 반응) + **이동 반사광 sheen** + **바닥/림 그림자**로 depth. 평면 스티커 금지.
- ⚠️ ffmpeg `colorkey` 자동 누끼는 **프린지/헤일로** 남음(데모용). 프로덕션 = **rembg/디자이너 정밀 알파 매트**.
