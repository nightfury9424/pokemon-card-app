...
## 4. Flutter 통합 스펙
### 4.0 자산 조건 · 알파 QA · 3D 규칙 — 필수 (합본 크롭·저해상도·가짜투명 금지)
**자산 조건**
- 각 템플릿 **독립 이미지**로 생성 — **3×3 합본 크롭 금지**(저해상도·엣지 손상).
- 원본 **≥ 1200×1985**, 최종 앱 자산 **780×1290**(@3x, 0.605).
- 배경 **실제 알파=0(RGBA)**. **가짜 투명 금지** — 체크무늬가 이미지에 구워진 RGB는 알파 아님(Image#18 실증: 975×1613 **RGB·alpha 없음**).
- 이미지에 **굽지 말 것**: 그림자·검은 무대·글자·로고·이동 대각 반사광.
- 밝은 팩(실버)은 밝은 배경 금지(매트 실패) → 투명 또는 어두운 대비 배경.
- 실루엣: 팩 외곽(상하 크림프 톱니·좌우 실링) 라인 따라, 톱니 사이도 alpha=0. `pack_top_strip`은 절취 라인 정확히 따라 분리.

**알파 QA (합격 기준 · 스크립트 `qa_alpha.py`·`matte_qa.py`)**
- 네 모서리 alpha=0 / 팩 외부 alpha=0 / **크림프 톱니 사이 alpha=0**
- 외곽 **흰색·회색·붉은 프린지 없음**
- **검정·흰색·빨강 배경 각각 합성**해 가장자리 검수
- partial(1..254 반투명) 비율 과다 = 소프트 헤일로 → 매트 재작업

**3D 규칙 (약하게만)**
- 기본 화면 **정면 유지**, 미세 원근 **최대 ±2~3°**만. 큰 기울기·강한 blur 금지.
- 두께감 = **측면 림라이트 + 작은 바닥 그림자 + 이동 sheen**(전부 Flutter 런타임, 이미지에 굽지 않음).
- 흐릿·비뚤어진 스티커처럼 보이면 **실패**.

**상태**: 현재 `225×315` A/B/C(합본 크롭) 및 `b_real`(Image#18 매트)은 **리뷰/파이프라인 데모 전용** — repo 최종 assets 금지. 독립 고해상도 투명 A/B/C를 위 QA 게이트 통과분만 `front/assets/oripa/`에 배치.
...
### 4.5 공통 마스터 구조 — 3종 동일 구조, **재질만 차이** (필수 lock)
세 템플릿(네이비·골드/홀로/실버)은 **표면 재질만** 다르고 **애니 구조는 완전 동일**. 안 그러면 Flutter 애니를 3벌 보정해야 함.
| 동일해야 함 (마스터) | 값 |
|---|---|
| 캔버스 | 780×1290 (@3x, 0.605) |
| 팩 외곽 실루엣·비율 | 동일(단일 실루엣) |
| 상단 크림프 하단 = **tear line Y** | 잠금(예: 높이의 ~9%) |
| 하단 크림프 상단 Y | 잠금(예: ~92%) |
| 입구 슬롯 | tear line~14% |
| **카드 상승 중심축** | x = 390(중앙) |
| 로고·POKEFOLIO·오리파명 안전영역 | §6과 동일(0–30% 비움 / 32–55% / 62–69% / 72–84%) |

| 달라도 됨 | 홀로 재질 · 네이비·골드 재질 · 실버 메탈릭 재질 (표면만) |

- **생성 규칙**: 3장을 **하나의 마스터 컴포지션의 재질 변형**으로 생성(독립 랜덤 생성 시 크림프·실루엣 어긋남). 방법 ①한 팩 확정 → img2img/리컬러로 **재질만 교체**(같은 실루엣/크림프/비율), 또는 ②동일 seed + material swap.
- **정렬 보정**: QA 통과분을 파이프라인에서 **마스터 프레임에 register**(팩 bounds·크림프·tear line을 잠근 Y로 정렬) → **단일 애니가 3종 모두 구동**. 템플릿별 별도 레이아웃·별도 절취 메커닉 **금지**.
