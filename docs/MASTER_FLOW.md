...
## ★★ 최상위 운영 원칙 — 백업 선행 (모든 작업 규칙보다 우선)
영향 범위가 큰 기능/데이터를 건드릴 때는 **본 작업 전에 백업과 롤백 경로를 먼저 확보**한다.
- **적용 대상**: DB 스키마·대량 데이터 / 가격 데이터·배치·계수 / 스캐너 FAISS·meta·모델·벡터 / 운영 프론트·백엔드 배포 / 브랜치 통합·rebase·대규모 merge / 카드 대량 등록·삭제·노출 / cron·동기화 파이프라인
- **필수 절차**: ①현재 운영 상태·기준값 기록 ②타임스탬프 백업 생성 ③백업 존재·크기·SHA(또는 복구가능 여부) 검증 ④롤백 명령·복구 순서 사전 작성 ⑤DRY_RUN 또는 별도 환경 검증 ⑥사용자 승인 후 본 작업 ⑦작업 후 전후값 비교 ⑧이상 시 즉시 롤백
- ★**백업 검증 전 운영 작업 시작 금지. 부분 백업·복구경로 불명확 상태로 진행 금지.**
...
2. **게시판 풀스택 + moderation 완성** — 글/댓글, 신고·차단, 금칙어, 운영자 삭제·제재, 어드민 모더레이션, 약관·24h 검토 정책, 탈퇴·차단 사용자 콘텐츠 처리. ★게시판은 **1.0.4에서 숨기지 않고 실제로 보이게 심사**(Apple 2.3.1 hidden-feature 회피). 각 글·댓글에서 신고/차단 직접 접근. (백엔드는 미리 prod 배포 가능 — 1.0.3 안 씀)
   ...
   - ★**1.0.4 최종 제품 구조(2026-06-23 확정)**: 게시판 = **`공지 | 자유` 2탭만**(기본=자유). 전체·거래후기·사기주의·Q&A 탭 및 작성 type 선택 제거. **공지**=`section=official`(notice/event/patch, 관리자만 작성·수정·삭제, 앱 작성/댓글 UI 없음, ★서버도 official 댓글 거부), **자유**=`type=free`(로그인 전원 작성·본인수정삭제·댓글/1단답글·모더레이션). type/컬럼은 삭제 않고 확장용 유지. **Q&A 채택=1.0.4 구현 제외**(스키마·설계 유지, 향후). 상세=`docs/BOARD_READ_BACKEND_DESIGN.md §1.0.4 최종 제품 구조`.
