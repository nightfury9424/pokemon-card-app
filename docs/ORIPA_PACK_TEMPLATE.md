...
| 영역 | Y (팩 높이 %) | 용도 | 제약 |
|------|--------------|------|------|
| 상단 절취·개봉·**카드 상승** exclusion | **0–30%** | **완전 비움 — 텍스트/로고/이미지 금지** | 뜯기고 **카드가 나오는 공간**. 브랜딩 걸치면 잘림/가림 |
| 매장 로고 / 대표이미지 | **32–55%** | `merchantLogoUrl` / `merchantArtworkUrl` | 박스 ≤ 폭 60% |
| **POKEFOLIO** 워드마크 | **62–69%** | 플랫폼 서명(고정) | 소형 |
| 오리파명 (+선택 서브라벨) | **72–84%** | `oripaTitle` (+ `SEALED ORIPA`) | ≤ 폭 72%, 최대 2줄 |
| 하단 크림프 | 90–100% | **비움** | 콘텐츠 금지 |
- **상단 30% 완전 비움**: 두 텍스트 블록(POKEFOLIO·오리파명)과 로고·이미지 **전부 하단**. 상단은 절취·개봉 입구·카드 상승 전용 → 글자/로고/이미지 금지.
- **상대 순서**: `POKEFOLIO`(62–69%) 위, 오리파명(72–84%) 아래 — 자리 교체 아님, 둘 다 하단.
- 좌우: 콘텐츠 x 10–90%(실링 seam 침범 금지). B 템플릿은 골드 프레임 내부(약 x12–88%)에 배치.
- **브랜딩 고정 규칙(중요)**: 로고·`POKEFOLIO`·오리파명은 전부 `pack_base`/`pack_open_body`의 **고정 오버레이**(하단). 뜯기는 `pack_top_strip`엔 **브랜딩 0(순수 포일)**. 절취/카드 상승 중에도 하단 텍스트는 **제자리 고정**. **3 템플릿 + 커스텀 업로드 모두 동일 배치·상단 exclusion(0–30%) 강제.**

## 7. 초기 템플릿 (승인본 **3종 전부** — 단일 팩 확정 아님)
> **결정 고정**: 카탈로그 = 아래 3종 모두. 네이비 하나로 좁히지 않는다. 사장님이 셋 중 선택 → 로고·오리파명 오버레이. 파트너센터엔 항상 3개 노출.
> 네이비 `b_edgecheck`(검정/흰색/빨강 합성)는 **누끼 품질 QA 테스트**일 뿐 제품 결정 아님. 누끼 파이프라인은 네이비 1차 검증 후 **홀로·실버에도 동일 적용**. **기존 디자인 유지, 재디자인 금지.**
