...
| **오리파명** (+선택 서브라벨) | **18–30%** | `oripaTitle` (+ `SEALED ORIPA`) | ≤ 폭 72%, 최대 2줄. **상품이 먼저** |
| 매장 로고 / 대표이미지 | **34–60%** | `merchantLogoUrl` / `merchantArtworkUrl` | 박스 ≤ 폭 60% |
| **POKEFOLIO** 워드마크 | **78–86%** | 플랫폼 서명(고정) | 소형·하단 |
| 하단 크림프 | 90–100% | **비움** | 콘텐츠 금지 |
- **정보 위계**: `POKEFOLIO`는 상단 헤드라인 아니라 **하단 플랫폼 서명**. 사용자가 뽑는 상품(오리파명)이 **위에서 먼저** 보인다.
- 좌우: 콘텐츠 x 10–90%(실링 seam 침범 금지). B 템플릿은 골드 프레임 내부(약 x12–88%)에 배치.
- **브랜딩 고정 규칙(중요)**: 오리파명·매장 로고·`POKEFOLIO`는 전부 `pack_base`/`pack_open_body`의 **고정 오버레이**. 뜯기는 `pack_top_strip`엔 **브랜딩 0(순수 포일)**. 절취 중 브랜드 요소는 **제자리 고정** — 조각에 안 따라감/안 잘림. **3 템플릿 + 커스텀 업로드 모두 동일 배치·상단 exclusion(0–15%) 강제.**
